...
{
	memset(Usart2_Rx_Buf,0,USART2_RBUFF_SIZE);//先清空缓冲区
  rt_kprintf("[NBiot_SendCmd] %s\r\n", cmd);
  rt_thread_delay(100);   /* 延时100个tick */		 		
  uart2_send_buff((u8*)cmd, strlen(cmd));
  rt_thread_delay(wait); 
  if (strcmp(Usart2_Rx_Buf, "") == 0) //返回值为空
  {
		 rt_kprintf("\r\n+++NONE!!\r\n");
    return 0;
  }
    if (strstr((char*)Usart2_Rx_Buf, reply))   //是否包含数据
    {
      rt_kprintf("\r\n%s+++YES\r\n", Usart2_Rx_Buf);
      return 1;
    }
...
void Get_IP(void) {
  ...
  while (!NBiot_SendCmd("AT+CGPADDR=1", "+CGPADDR: 1", 500)) //循环连接，直到连接到服务器
  {
    try_time2 += 1;
